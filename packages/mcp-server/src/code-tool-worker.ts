...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Fashn)`
    : `const run: (${functionSource.client}: Fashn) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(['client.predictions.run', 'client.predictions.status'], {
  threshold: 1,
  shouldSort: true,
});
