...
import { ClientOptions } from 'fashn';

const prompt = `Runs JavaScript code to interact with the Fashn API.
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { FASHN_API_KEY: requireValue(readEnv('FASHN_API_KEY') ?? client.apiKey, 'set FASHN_API_KEY environment variable or provide apiKey client option'), FASHN_BASE_URL: readEnv('FASHN_BASE_URL') ?? client.baseURL ?? undefined };
